var DemoApi = (() => {
  ...
  // miniprogram/lib/core.js
  var require_core = __commonJS({
    "miniprogram/lib/core.js"(exports, module) {
      ...
      var require_geo = __commonJS2({
        "shared/geo.js"(exports2, module2) {
          var PI = Math.PI;
          ...
          function transform(x, y, lat) {
            let v = lat ? -100 + 2 * x + 3 * y + 0.2 * y * y + 0.1 * x * y + 0.2 * Math.sqrt(Math.abs(x)) : 300 + x + 2 * y + 0.1 * x * x + 0.1 * x * y + 0.1 * Math.sqrt(Math.abs(x));
            v += (20 * Math.sin(6 * x * PI) + 20 * Math.sin(2 * x * PI)) * 2 / 3;
            const z = lat ? y : x;
            v += (20 * Math.sin(z * PI) + 40 * Math.sin(z / 3 * PI)) * 2 / 3;
            v += ((lat ? 160 : 150) * Math.sin(z / 12 * PI) + (lat ? 320 : 300) * Math.sin(z * PI / 30)) * 2 / 3;
            return v;
          }
          function wgsToGcj(p) {
            ...
            return {
              ...p,
              latitude: p.latitude + transform(p.longitude - 105, p.latitude - 35, true) * 180 / (6378245 * (1 - 0.006693421622965943) / (magic * root) * PI),
              longitude: p.longitude + transform(p.longitude - 105, p.latitude - 35, false) * 180 / (6378245 / root * Math.cos(rad) * PI)
            };
          }
          function gcjToWgs(p) {
            if (!validPoint(p)) throw new Error("\u65E0\u6548\u7684\u7ECF\u7EAC\u5EA6");
          ...
          function distance(a, b) {
            const r = PI / 180, dlat = (b.latitude - a.latitude) * r, dlon = (b.longitude - a.longitude) * r;
            const h = Math.sin(dlat / 2) ** 2 + Math.cos(a.latitude * r) * Math.cos(b.latitude * r) * Math.sin(dlon / 2) ** 2;
            return 63710088e-1 * 2 * Math.atan2(Math.sqrt(h), Math.sqrt(Math.max(0, 1 - h)));
          }
          function stats(segments) {
            let meters = 0, gain = 0, elevation = true, count = 0;
          ...
          function project(segments, width = 320, height = 170, padding = 24) {
            ...
            const points = segments.map(
              (s) => s.map((p) => ({ x: p.longitude * Math.cos(lat0), y: -p.latitude }))
            );
            const ps = points.flat(), xs = ps.map((p) => p.x), ys = ps.map((p) => p.y);
            const minx = Math.min(...xs), maxx = Math.max(...xs), miny = Math.min(...ys), maxy = Math.max(...ys);
            const scale = Math.min(
              (width - padding * 2) / Math.max(maxx - minx, 1e-5),
              (height - padding * 2) / Math.max(maxy - miny, 1e-5)
            );
            return points.map(
              (s) => s.map((p) => ({
          ...
          module2.exports = { validPoint, wgsToGcj, gcjToWgs, distance, stats, project };
        }
      });
          ...
          (() => {
            ...
            class it {
              ...
              _matchSegment(t2, e2, i2) {
                ...
                if (void 0 !== t2.position) {
                  if (!i2) return false;
                  const n2 = (_a = e2.counter) != null ? _a : 0;
            ...
            class rt {
              ...
              add(t2) {
                ...
                const e2 = t2.length, i2 = t2.segments[t2.segments.length - 1], n2 = i2 == null ? void 0 : i2.tag;
                if (n2 && "*" !== n2) {
                  const i3 = `${e2}:${n2}`;
            ...
            class xt {
              constructor(t2 = {}) {
                var _a, _b;
                var e2;
                this._limit = t2.limit || {}, this._maxTotalExpansions = this._limit.maxTotalExpansions || 0, this._maxExpandedLength = this._limit.maxExpandedLength || 0, this._postCheck = "function" == typeof t2.postCheck ? t2.postCheck : (t3) => t3, this._limitTiers = (e2 = (_a = this._limit.applyLimitsTo) != null ? _a : dt) && e2 !== dt ? e2 === ft ? /* @__PURE__ */ new Set([ft]) : e2 === ut ? /* @__PURE__ */ new Set([ut]) : Array.isArray(e2) ? new Set(e2) : /* @__PURE__ */ new Set([dt]) : /* @__PURE__ */ new Set([dt]), this._numericAllowed = (_b = t2.numericAllowed) != null ? _b : true, this._baseMap = ht(ot, t2.namedEntities || null), this._externalMap = /* @__PURE__ */ Object.create(null), this._inputMap = /* @__PURE__ */ Object.create(null), this._totalExpansions = 0, this._expandedLength = 0, this._removeSet = new Set(t2.remove && Array.isArray(t2.remove) ? t2.remove : []), this._leaveSet = new Set(t2.leave && Array.isArray(t2.leave) ? t2.leave : []);
                const i2 = (function(t3) {
                  var _a2, _b2;
                  if (!t3) return { xmlVersion: 1, onLevel: gt.allow, nullLevel: gt.remove };
                  const e3 = 1.1 === t3.xmlVersion ? 1.1 : 1, i3 = (_a2 = gt[t3.onNCR]) != null ? _a2 : gt.allow, n2 = (_b2 = gt[t3.nullNCR]) != null ? _b2 : gt.remove;
                ...
                for (; s2 < n2; ) {
                  ...
                  if (this._removeSet.has(p3)) c2 = "", void 0 === h2 && (h2 = dt);
                    ...
                    if (35 === p3.charCodeAt(0)) {
                      ...
                      c2 = t3 == null ? void 0 : t3.value, h2 = t3 == null ? void 0 : t3.tier;
                    }
                  }
                  if (void 0 !== c2) {
            ...
            function Ct(t2, e2) {
              var _a;
              const i2 = (_a = e2.label) != null ? _a : "CUSTOM";
      ...
      var require_gpx = __commonJS2({
        "shared/gpx.js"(exports2, module2) {
          var { XMLParser, XMLValidator } = require_fxp();
          var { validPoint } = require_geo();
          var MAX_POINTS = 8e3;
          var array = (x) => x == null ? [] : Array.isArray(x) ? x : [x];
          function readPoint(p) {
            const q = { latitude: Number(p["@_lat"]), longitude: Number(p["@_lon"]) };
            if (p["@_lat"] == null || p["@_lon"] == null || String(p["@_lat"]).trim() === "" || String(p["@_lon"]).trim() === "" || !validPoint(q))
          ...
          function parseGpx(text) {
            ...
            const g = xml.gpx;
            let segments = array(g.trk).flatMap((t) => array(t.trkseg).map((s) => array(s.trkpt).map(readPoint))).filter((s) => s.length);
            if (!segments.length)
              segments = array(g.rte).map((r) => array(r.rtept).map(readPoint)).filter((s) => s.length);
            ...
            const waypoints = array(g.wpt).slice(0, 100).map((p) => ({
              ...readPoint(p),
              name: typeof p.name === "string" ? p.name.slice(0, 60) : "\u9014\u7ECF\u70B9"
            }));
            const title = g.metadata && g.metadata.name || ((_a = array(g.trk)[0]) == null ? void 0 : _a.name) || ((_b = array(g.rte)[0]) == null ? void 0 : _b.name) || "\u5BFC\u5165\u7684\u8DEF\u4E66";
            return {
          ...
          function exportGpx(route) {
            ...
            const point = (tag, p) => `<${tag} lat="${p.latitude.toFixed(7)}" lon="${p.longitude.toFixed(7)}">${Number.isFinite(p.elevation) ? `<ele>${p.elevation}</ele>` : ""}${p.name ? `<name>${escapeXml(p.name)}</name>` : ""}</${tag}>`;
...
<gpx version="1.1" creator="Roadbook" xmlns="http://www.topografix.com/GPX/1/1"><metadata><name>${escapeXml(route.title)}</name></metadata>${(route.waypoints || []).map((p) => point("wpt", p)).join("")}<trk><name>${escapeXml(route.title)}</name>${segments.map((s) => `<trkseg>${s.map((p) => point("trkpt", p)).join("")}</trkseg>`).join("")}</trk></gpx>`;
          ...
          module2.exports = { parseGpx, exportGpx, MAX_POINTS };
        }
      });
      var require_routes = __commonJS2({
        "shared/routes.js"(exports2, module2) {
          var geo = require_geo();
          var { MAX_POINTS } = require_gpx();
          var MODES = { cycling: "\u9A91\u884C", walking: "\u6B65\u884C", driving: "\u81EA\u9A7E" };
          ...
          function validateRoute(input) {
            ...
            if (typeof input.description !== "string" || input.description.length > 2e3)
          ...
          function routeStops(route) {
            const { start, end } = endpoints(route), wp = route.waypoints || [];
            ...
            return [
              ...
              ...wp.filter((_, i) => i !== startIndex && i !== endIndex).map((p) => ({ ...p, role: route.source === "gpx" ? "\u6807\u8BB0\u70B9" : "\u9014\u7ECF\u70B9" })),
              { ...end, name: endIndex >= 0 ? wp[endIndex].name : "\u8DEF\u4E66\u7EC8\u70B9", role: "\u7EC8\u70B9" }
            ];
          }
          function amapUrl(route, options = {}) {
            const { start, end } = endpoints(route), a = geo.wgsToGcj(start), b = geo.wgsToGcj(end);
            const stops = routeStops(route), wp = route.waypoints || [];
            const pair = (p, name) => `${p.longitude.toFixed(6)},${p.latitude.toFixed(6)},${name.replace(/[,|]/g, " ")}`;
            const params = {
            ...
            return "https://uri.amap.com/navigation?" + Object.keys(params).map((k) => encodeURIComponent(k) + "=" + encodeURIComponent(params[k])).join("&");
          }
          function summarize(route) {
            ...
            return {
              ...
              distanceLabel: (route.distanceMeters / 1e3).toFixed(1),
              modeLabel: MODES[route.mode]
            };
          }
          module2.exports = { MODES, validateRoute, endpoints, routeStops, amapUrl, summarize };
        }
      });
      module.exports = { ...require_geo(), ...require_gpx(), ...require_routes() };
    }
  });

  // miniprogram/lib/fixtures.js
  var require_fixtures = __commonJS({
    "miniprogram/lib/fixtures.js"(exports, module) {
      var fixtureData = [
        {
          id: "demo-river",
          title: "\u6CBF\u7740\u6C5F\u98CE\uFF0C\u6162\u6162\u9A91",
          city: "\u4E0A\u6D77",
          mode: "cycling",
          description: "\u4E00\u6BB5\u7559\u7ED9\u5468\u672B\u7684\u6EE8\u6C5F\u65F6\u5149\u3002\u628A\u5496\u5561\u3001\u6C5F\u98CE\u548C\u65E5\u843D\u653E\u8FDB\u540C\u4E00\u6761\u8DEF\u7EBF\u3002\n\n\u8FD9\u662F\u7528\u4E8E\u4F53\u9A8C\u529F\u80FD\u7684\u793A\u4F8B\u8F68\u8FF9\uFF0C\u672A\u7ECF\u5B9E\u5730\u6838\u9A8C\uFF0C\u8BF7\u52FF\u6309\u793A\u610F\u7EBF\u9A91\u884C\u3002",
          coords: [
            [121.485, 31.245],
            [121.482, 31.239],
            [121.485, 31.231],
        ...
        },
        {
          ...
          city: "\u676D\u5DDE",
          mode: "cycling",
          description: "\u7ED5\u8FC7\u6811\u5F71\uFF0C\u627E\u4E00\u5904\u6E56\u8FB9\u6B47\u811A\u3002\u9002\u5408\u7528\u6765\u4F53\u9A8C\u73AF\u7EBF\u7684\u5C55\u793A\u4E0E GPX \u5BFC\u51FA\u3002\n\n\u6F14\u793A\u6570\u636E\uFF0C\u8DEF\u7EBF\u4EC5\u4F5C\u793A\u610F\uFF0C\u4E0D\u4F5C\u4E3A\u901A\u884C\u4F9D\u636E\u3002",
          coords: [
            [120.148, 30.252],
            [120.139, 30.259],
            [120.126, 30.258],
        ...
        },
        {
          ...
          city: "\u82CF\u5DDE",
          mode: "walking",
          description: "\u4ECE\u4E00\u6761\u8001\u8857\u8D70\u5230\u53E6\u4E00\u6761\u5DF7\u5B50\uFF0C\u6CBF\u9014\u7559\u4E9B\u7A7A\u767D\u7ED9\u5076\u9047\u7684\u5C0F\u5E97\u3002\n\n\u6F14\u793A\u6570\u636E\uFF0C\u8DEF\u7EBF\u4EC5\u4F5C\u793A\u610F\uFF0C\u4E0D\u4F5C\u4E3A\u901A\u884C\u4F9D\u636E\u3002",
          coords: [
            [120.62, 31.316],
            [120.622, 31.313],
            [120.625, 31.313],
  ...
  var require_config = __commonJS({
    "miniprogram/config.js"(exports, module) {
      module.exports = { mode: "demo" };
    }
  });

  // miniprogram/services/api.js
  var require_api = __commonJS({
    "miniprogram/services/api.js"(exports, module) {
      var core = require_core();
      var fixtures = require_fixtures();
      var config = require_config();
      var KEY = "roadbook-demo-v1";
      function initial() {
        return {
          user: null,
          routes: fixtures.map((f, i) => {
            const segments = [
      ...
      async function demo(action, data = {}) {
        ...
        if (action === "profile") {
          ...
          user.nickname = String(data.nickname || "").trim().slice(0, 30);
        ...
        if (action === "list") {
          let list = s.routes.filter(
            (r) => data.mine ? user && r.ownerId === user.id : r.visibility === "public"
          );
        ...
        if (action === "detail") {
          const r = s.routes.find((r2) => r2._id === data.id);
        ...
        if (action === "save") {
          if (!user) throw new Error("\u8BF7\u5148\u767B\u5F55");
          let old = data.id ? s.routes.find((r2) => r2._id === data.id) : null;
        ...
        if (action === "delete") {
          ...
          s.routes = s.routes.filter((r2) => r2._id !== data.id);
          save(s);
          return true;
        }
        if (action === "plan") {
      ...
      function report(error) {
        ...
        if (!/cancel/.test(message)) wx.showToast({ title: message, icon: "none", duration: 3e3 });
      }
